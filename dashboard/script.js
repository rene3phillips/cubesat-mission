...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchTelemetry() {
        ...
        try {
            ...
            hum.textContent = (data.HUM !== undefined ? data.HUM.toFixed(1) : "--") + " %";
    ...
    async function fetchMissions() {
        try {
            const response = await fetch('/missions'); // Backend endpoint
            ...
            missions.forEach(mission => {
                if (!uniqueMissions.has(mission.mission_name)) {
                    ...
                    logCard.addEventListener('mouseenter', () => {
                        chartImage.src = `/missions/${encodeURIComponent(mission.mission_name)}/chart`;
                    });

                    logCard.addEventListener('mouseleave', () => {
                        chartImage.src = 'images/sample-graph.png'; // fallback image
                    });


                    // Click event navigates to new page
                    logCard.addEventListener('click', () => {
                        // Navigate to mission-specific page
                        window.location.href = `/missions/${encodeURIComponent(mission.mission_name)}`;
                    });

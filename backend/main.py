...
def list_missions(db: Session = Depends(database.get_db)):
    ...
    return [
        {"id": m.id, "mission_name": m.mission_name, "temp": m.temp, "hum": m.hum, "timestamp": m.timestamp}
        for m in missions
    ]

@app.get("/missions/{mission_name}")
def get_mission_data(mission_name: str, db: Session = Depends(get_db)):
    telemetry_data = get_mission_by_name(db, mission_name)
...
def get_mission_chart(mission_name: str, db: Session = Depends(database.get_db)):
    ...
    data = db.query(models.Mission).filter(models.Mission.mission_name == mission_name).all()
    if not data:
        raise HTTPException(status_code=404, detail="Mission not found")
    ...
    ax.set_xticklabels([ts.strftime("%H:%M:%S") for ts in timestamps[::max(1,len(timestamps)//10)]], color='whitesmoke', rotation=45)
